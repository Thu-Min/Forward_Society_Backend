...
<div class="w-[80%] h-max  mx-auto ">
    <div class="mt-5 md:mt-0 ">
        <form action="{{route('events.update',['event'=>$event->slug])}}" class="py-4 px-5 " method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="py-4 px-5 shadow bg-white sm:rounded-md  ">
                ...
                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    <div>
                        ...
                        <div class="mt-1">
                            <input name="name" class="py-2 px-4 border outline-none shadow-sm focus:ring-blue-400 focus:border-blue-400 mt-1 block w-full sm:text-sm border-gray-300 rounded-md" value="{{old('name',$event->name)}}" placeholder="Event Title..." />
                        </div>
                        @error('name')
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input name="slug" class="py-2 px-4 border outline-none shadow-sm focus:ring-blue-400 focus:border-blue-400 mt-1 block w-full sm:text-sm border-gray-300 rounded-md" value="{{old('slug',$event->slug)}}" placeholder="Event Title..." />
                        </div>
                        @error('slug')
                        <div class="text-red-600 text-xs">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea id="description" name="description" rows="3" class="py-2 px-4 outline-none border shadow-sm focus:ring-blue-400 focus:border-blue-400 mt-1 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Event Description....">{{old('description',$event->description)}}</textarea>
                        </div>
                        @error('description')
                        <div class="text-red-600 text-xs">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea id="description_detail" name="description_detail" rows="3" class="py-2 px-4 outline-none border shadow-sm focus:ring-blue-400 focus:border-blue-400 mt-1 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Event Description....">{{old('description_detail',$event->description_detail)}}</textarea>
                        </div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option @if((old('status') && old('status')=='upcoming' ) || $event->status == 'upcoming') selected @endif value="upcoming">Upcoming</option>
                            <option @if((old('status') && old('status')=='ongoing' ) || $event->status == 'ongoing') selected @endif value="ongoing">Ongoing</option>
                            <option @if((old('status') && old('status')=='finished' ) || $event->status == 'finished') selected @endif value="finished">Finished</option>
                        </select>
                        @error('status')
                        <div class="text-red-600 text-xs">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="">
                        <img src='{{asset("/storage/$event->thumbnail")}}' alt="" class="object-cover h-30 w-50">
                        <label for="about" class="block text-md font-medium text-gray-700">
                            Event Thumbnail
                        </label>
                    ...
                    <div class="flex justify-between items-center">
                        <div class="w-[45%]">
                            ...
                            <div class="mt-1">
                                <input name="instructor" value="{{old('instructor',$event->instructor)}}" class="py-2 px-4 border outline-none shadow-sm focus:ring-blue-400 focus:border-blue-400 mt-1 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Event Instructor..." />
                            </div>
                            @error('instructor')
                            <div class="text-red-600 text-xs">{{ $message }}</div>
                            @enderror
                            ...
                            <select id="categories" name="categories[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" multiple>
                                ...
                                <option value="{{$category->id}}"
                                    ...
                                    @foreach($eventCategories as $eventCategory)
                                        ...
                                        {{$eventCategory->id == $category->id ? "selected" : ''}}
                                    @endforeach
                                    @endif
